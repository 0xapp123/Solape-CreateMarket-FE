...
function WalletSelectorPanelItem({
  ...
  return (
    ...
    >
      ...
      <Row className="flex-wrap items-center justify-between grow">
        <div className="text-base font-bold text-white mobile:text-sm">{wallet.adapter.name}</div>
        {detected && !isMobile && (
          <Badge
...
function SimulateWallet({ onClick }: { onClick?(): void }) {
  ...
  return (
    <Col className="p-6 mobile:py-3 mobile:px-4 flex-grow ring-inset ring-1.5 mobile:ring-1 ring-[rgba(171,196,255,.5)] rounded-3xl mobile:rounded-xl items-center gap-3 m-8 mt-2 mb-4">
      <div className="text-base font-bold text-white mobile:text-sm">Simulate Wallet Address</div>
      <Input
        className="w-full"
  ...
  return (
    ...
    >
      <Row className="items-center justify-between p-8 mobile:p-4">
        <div className="text-xl font-semibold text-white mobile:text-lg">Connect your wallet to SOLAPE</div>
        <Icon className="text-[#ABC4FF] cursor-pointer" heroIconName="x" onClick={close} />
      </Row>

      {/* Disclaimer */}
      <div className="grow text-sm mobile:text-xs leading-normal text-[#abc4ffb3] rounded p-4 mobile:p-2 mobile:px-4 mb-6 mobile:mb-4 mx-8 mobile:mx-6 bg-[#161a1a]">
